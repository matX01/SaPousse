...
void Debugger::DisplayMenu(){
    ...
    switch(MenuID){
        ...
        break;
        case 23:
            DisplayerToken[2] = !DisplayerToken[2];
            GoBack();
...
void Debugger::actualise(){
     //Serial.println(GPSHandling::GPSSentence);
    if(DebuggingToolActivated){
        ...
        if(DisplayerToken[0]){
            ...
            if(IMUData::DataReady){
                ...
                for(int i = 0;i < 10;i ++){
                    ...
                    Serial.print(IMUData::OutputFloat[i]);
                    Serial.print(",");
        ...
        if(DisplayerToken[2]){
                ...
                if((*GPSDev.GetValues()).DataReady){

                    Serial.println(GPSHandling::GPSSentence);
                   
                }

                (*GPSDev.GetValues()).DataReady = false;
